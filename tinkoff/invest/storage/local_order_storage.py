...
class OrdersStorage(IOrdersStorage):
    ...
    def add(self, order: OrderState) -> None:
        if order.order_id not in self._orders:
            ...
            raise KeyError(f"Cannot add order_id={order.order_id}, order is already in storage")

    def delete(self, order_id: str) -> None:
        if order_id in self._orders:
            del self._orders[order_id]
        else:
            raise KeyError(f"Cannot delete order_id={order_id}, order is not in storage")
