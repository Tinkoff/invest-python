...
from tinkoff.invest import InstrumentIdType, CurrencyResponse, InstrumentStatus, \
    CurrenciesResponse, BondResponse, BondsResponse, EtfResponse, EtfsResponse, \
    FutureResponse, FuturesResponse, ShareResponse, SharesResponse, Etf, Future, Share, \
    Bond, Currency
...
class InstrumentsCache(IInstrumentsGetter):
    def __init__(self, settings: InstrumentsCacheSettings, instruments_service: InstrumentsService):
        ...
        self._cache = TTLCache(maxsize=len(self._instruments_methods), ttl=self._settings.ttl.total_seconds())
        self._refresh_cache()
    ...
    def _assert_cache(self):
        assert self._cache.keys() == set(map(lambda f: f.__name__, self._instruments_methods)), f'Cache does not have all instrument types {self._cache}'

    def _get_instrument_storage(self, get_instruments_method: InstrumentsResponseCallable) -> InstrumentStorage[InstrumentResponse, InstrumentsResponse]:
        ...
        if storage is not None:
            ...
            return storage
        logger.debug('Storage for key %s not found, creating new storage with ttl=%s', storage_key, self._cache.ttl)
        instruments_response = get_instruments_method(instrument_status=InstrumentStatus.INSTRUMENT_STATUS_ALL)
        storage = InstrumentStorage(
            instruments_response=instruments_response
        )
        self._cache[storage_key] = storage
        return storage

    def shares(self, *, instrument_status: InstrumentStatus = InstrumentStatus(0)) -> SharesResponse:
        storage = cast(
    ...
    def share_by(self, *, id_type: InstrumentIdType = InstrumentIdType(0),
                 class_code: str = "", id: str = "") -> ShareResponse:
        storage = cast(
    ...
    def futures(self, *, instrument_status: InstrumentStatus = InstrumentStatus(0)) -> FuturesResponse:
        storage = cast(
    ...
    def future_by(self, *, id_type: InstrumentIdType = InstrumentIdType(0),
                  class_code: str = "", id: str = "") -> FutureResponse:
        storage = cast(
    ...
    def etfs(self, *,
             instrument_status: InstrumentStatus = InstrumentStatus(0)) -> EtfsResponse:
        storage = cast(
    ...
    def etf_by(self, *, id_type: InstrumentIdType = InstrumentIdType(0),
               class_code: str = "", id: str = "") -> EtfResponse:
        storage = cast(
    ...
    def bonds(self, *, instrument_status: InstrumentStatus = InstrumentStatus(
        0)) -> BondsResponse:
        storage = cast(
    ...
    def bond_by(self, *, id_type: InstrumentIdType = InstrumentIdType(0),
                class_code: str = "", id: str = "") -> BondResponse:
        storage = cast(
    ...
    def currencies(self, *, instrument_status: InstrumentStatus = InstrumentStatus(
        0)) -> CurrenciesResponse:
        storage = cast(
    ...
    def currency_by(self, *, id_type: InstrumentIdType = InstrumentIdType(0),
                    class_code: str = "", id: str = "") -> CurrencyResponse:
        storage = cast(
