...
def main():
    with Client(TOKEN) as client:
        ...
        # setting the percentage by which the takeprofit stop order should be set below the last price
        percent_down = 5
        ...
        print(f'min_price_increment = {min_price_increment}, number_digits_after_point={number_digits_after_point}')

        # calculation of the price for instrument which is divisible to min price increment
        calculated_price = round(calculated_price / min_price_increment) * min_price_increment
        print(f'let\'s send stop order at price = {calculated_price:.{number_digits_after_point}f} divisible to '
        ...
        try:
            ...
            logger.error(f'Posting trade takeprofit order failed. Exception: {error}')
